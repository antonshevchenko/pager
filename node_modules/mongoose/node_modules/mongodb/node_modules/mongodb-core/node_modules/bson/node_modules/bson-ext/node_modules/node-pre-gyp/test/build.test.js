...
var runtime = semver.parse(process.version).major < 1 ? 'node' : 'iojs';
...
function run(prog,command,args,app,opts,cb) {
    if(prog === 'node-pre-gyp'){
        prog = runtime + ' ' + path.resolve(__dirname + '/../bin/node-pre-gyp.js');
    }
...
describe('simple build and test', function() {
    ...
    var app = {"name": "app2", "args":'--toolset=cpp11 --custom_include_path="../include"'};
    ...
    it(app.name + ' passes tests ' + app.args, function(done) {
        run(runtime,'index.js','', app, {env : process.env, cwd: path.join(__dirname,app.name)}, function(err,stdout,stderr) {
            if (err) return on_error(err,stdout,stderr);
            assert.equal(stderr,'');
            ...
            it(app.name + ' passes tests ' + app.args, function(done) {
                run(runtime,'index.js','', app, {env : process.env, cwd: path.join(__dirname,app.name)}, function(err,stdout,stderr) {
                    ...
                    if (app.name == 'app2') {
                        ...
                        assert.notEqual(stdout,'', 'we expect some kind of output');
                    }
                    done();
                });
            });
